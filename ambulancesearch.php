...
<head>
    ...
    <title>الصيدليات</title>
		...
		mysql_query("SET NAMES 'utf8'");
		$ambAddress=$_POST['ampAddress'];
		if (isset($ambAddress))
		{
			$q=mysql_query("select * from amb where amb_address like '%$ambAddress%'",$db);	
		}
			 mysql_query("SET NAMES 'utf8'");
			 //$query = mysql_query("select * from doctors where price <100");
			 while ($row = mysql_fetch_array($q)){
			 $address=$row['amb_address'];
			 $lat=$row['amb_lat'];
			 $lon=$row['amb_lang'];
			 
			 echo ("addMarker($lat, $lon,'العنوان:$address');\n");
			 }
			 ?>
...
<body  onLoad="initMap()" id="page-top" data-spy="scroll" data-target=".navbar-fixed-top">
        ...
        <div class="container">
            ...
            <div class="collapse navbar-collapse navbar-right navbar-main-collapse">
                <ul class="nav navbar-nav">
                    ...
                    <li>
                        <a class="page-scroll" href="#pharmacy"><h5>الصيدليات</h5></a>
                    </li>
    ...
    <!-- Result Section -->
    <section id="pharmacy" class="content-section text-center">
