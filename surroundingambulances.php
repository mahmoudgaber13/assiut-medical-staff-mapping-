...
$i=1;
$q=mysql_query("delete from sur_amb",$db)or die(mysql_error());
while($i<=17)
{
	
	$q=mysql_query("select amb_lat,amb_lang from amb where amb_id= $i",$db)or die(mysql_error());
	...
	$qq=mysql_query("Insert into sur_amb values($i , $dist)",$db)or die(mysql_error());
...
<head>
	...
	<style type="text/css">
			...
			<script src="https://maps.googleapis.com/maps/api/js?key=AIzaSyCoNb2pr6QCJegnk6JbowePMYZ1Zvj_rxo"type="text/javascript"></script>
			 ...
			 $q=mysql_query("select * from sur_amb,amb where sur_amb.id=amb.amb_id order by dist limit 5",$db);
			 mysql_query("SET NAMES 'utf8'");
			 ...
			 $lat=$row['amb_lat'];
			 $lon=$row['amb_lang'];
			 $desc=$row['amb_address'];
			 echo ("addMarker($lat, $lon,'<b>العنوان:$desc</b>');\n");
        ...
        <div class="container">
            ...
            <div class="collapse navbar-collapse navbar-right navbar-main-collapse">
                <ul class="nav navbar-nav">
					...
					<li>
						<a class="page-scroll" href="#download">بيانات الاسعاف</a>
					</li>
					<li >
						<a  class="page-scroll" href="#about">أقرب اسعافات</a>
					</li>
					<li >			
